...
NSString *KRSHTTPKeyEncode(NSString *key) {
  ...
  NSString *escaped =
      (NSString *)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(
          kCFAllocatorDefault, (CFStringRef)key, NULL,
...
NSString *KRSHTTPValueEncode(NSString *value) {
  ...
  NSString *escaped =
      (NSString *)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(
          kCFAllocatorDefault, (CFStringRef)value, NULL,
          (CFStringRef) @"!*'();:@&=+$,/?%#[]", kCFStringEncodingUTF8));
  return escaped;
}
...
NSString *KRSHTTPValue(NSString *keyPath, id value) {
  ...
  if ([value isKindOfClass:[NSString class]]) {
    output = [KRSQueryDataSerializer stringFromUnescapedString:value
                                                   withKeyPath:keyPath];
  } else if ([value isKindOfClass:[NSNumber class]]) {
    output =
        [KRSQueryDataSerializer stringFromUnescapedString:[value stringValue]
                                              withKeyPath:keyPath];
  } else if ([value isKindOfClass:[NSArray class]]) {
    output = [KRSQueryDataSerializer stringFromArray:value withKeyPath:keyPath];
  } else if ([value isKindOfClass:[NSDictionary class]]) {
    output =
        [KRSQueryDataSerializer stringFromDictionary:value withKeyPath:keyPath];
  }
...
+ (NSString *)stringFromUnescapedString:(NSString *)string
                            withKeyPath:(NSString *)keyPath {
